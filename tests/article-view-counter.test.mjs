...
test("the decision fatigue article is accepted as published", async () => {
  assert.equal(PUBLISHED_ARTICLE_SLUGS.has(DECISION_FATIGUE_SLUG), true);
  const result = await json(
    await callApi(new MockD1(), "GET", undefined, undefined, DECISION_FATIGUE_SLUG)
  );
  assert.equal(result.status, 200);
...
test("the outcome bias article is accepted as published", async () => {
  assert.equal(PUBLISHED_ARTICLE_SLUGS.has(OUTCOME_BIAS_SLUG), true);
  const result = await json(
    await callApi(new MockD1(), "GET", undefined, undefined, OUTCOME_BIAS_SLUG)
  );
  assert.equal(result.status, 200);
  assert.equal(result.body.slug, OUTCOME_BIAS_SLUG);
});

test("the risk perception article is accepted as published", async () => {
  assert.equal(PUBLISHED_ARTICLE_SLUGS.has(RISK_PERCEPTION_SLUG), true);
  const result = await json(
    await callApi(new MockD1(), "GET", undefined, undefined, RISK_PERCEPTION_SLUG)
  );
  assert.equal(result.status, 200);
  assert.equal(result.body.slug, RISK_PERCEPTION_SLUG);
});

test("the planning fallacy article is accepted as published", async () => {
  assert.equal(PUBLISHED_ARTICLE_SLUGS.has(PLANNING_FALLACY_SLUG), true);
  const result = await json(
    await callApi(new MockD1(), "GET", undefined, undefined, PLANNING_FALLACY_SLUG)
  );
  assert.equal(result.status, 200);
  assert.equal(result.body.slug, PLANNING_FALLACY_SLUG);
});
